...
async def _start_desktop() -> None:
    ...
    async def _spawn(*cmd: str) -> asyncio.subprocess.Process:
        argv = (
            ["sudo", "-u", _DESKTOP_USER, "env", f"HOME={home}", f"DISPLAY={display}", *cmd]
            if drop
            else list(cmd)
        )
    ...
    _desktop_procs.append(
        await _spawn(
            "x11vnc", "-display", display, "-rfbport", str(_VNC_PORT),
            "-forever", "-shared", "-nopw", "-localhost",
        )
    )
    if shutil.which("websockify"):  # optional noVNC debug viewer on :6080
        _desktop_procs.append(
            await _spawn("websockify", "--web", "/usr/share/novnc", "6080", f"localhost:{_VNC_PORT}")
        )
